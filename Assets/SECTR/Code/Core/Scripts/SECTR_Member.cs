...
{	
	...
	public struct Child
	{
		public void Init(GameObject gameObject, Renderer renderer, Light light, Terrain terrain, SECTR_Member member, bool dirShadowCaster, Vector3 shadowVec)
		{
			...
			#else
			shadowLight = this.light && light.shadows != LightShadows.None && (!light.alreadyLightmapped || (LightmapSettings.lightmapsMode == LightmapsMode.Dual));
			#endif
			rendererCastsShadows = this.renderer && renderer.castShadows && (renderer.lightmapIndex == -1 || LightmapSettings.lightmapsMode == LightmapsMode.Dual);
			terrainCastsShadows = this.terrain && terrain.castShadows && (terrain.lightmapIndex == -1 || LightmapSettings.lightmapsMode == LightmapsMode.Dual);
